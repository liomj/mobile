...
if ($uname == '' || $pass == '') {
    redirect_header(XOOPS_URL . '/mobile/index.php', 1, _US_INCORRECTLOGIN);
}
...
if (false !== $user) {
    if (0 == $user->getVar('level')) {
        redirect_header(XOOPS_URL . '/mobile/index.php', 5, _US_NOACTTPADM);
    }
    ...
    if (!empty($redirect) && !strpos($redirect, 'register')) {
        ...
        $url = XOOPS_URL . '/main.php';
    }

    redirect_header($url, 1, sprintf(_US_LOGGINGU, $user->getVar('uname')), false);
} elseif (empty($redirect)) {
    redirect_header(XOOPS_URL . '/mobile/index.php', 5, $xoopsAuth->getHtmlErrors());
} else {
    redirect_header(XOOPS_URL . '/mobile/index.php?xoops_redirect=' . urlencode($redirect), 5, $xoopsAuth->getHtmlErrors(), false);
}
exit();
